...
def metadata(path, author, header, created, topic, source):
    header = header.replace('?','').replace('/','')
    
    create_folders(path, author, header, created, topic, source)
...
def create_folders(path, author, header, created, topic, source):
    row = '@au %s\n@da %s\n@topic %s\n@url %s\n'
    if not os.path.exists(path):
        os.makedirs(path)
    
    file2 = open(path + os.sep + header + '.txt', 'w', encoding = 'utf-8')
    file2.write(row % (author, created, topic, source))
    file2.close()
...
def main():
    ...
    for s in source:
        ...
        for articleUrl in sourceArticleUrl:
            ...
            metadata(path, author, header, created, topic, articleUrl)
            time.sleep(2)
